...
if __name__ == "__main__":
    ...
    os.environ["VLLM_USE_V1"] = "0"  # Per-request profiling is not in v1 engine, so use v0 (ONLY OPTION FOR XPU)
    multiprocessing.set_start_method("fork", force=True) # For power monitoring root access
    ...
    ray.init(
        num_cpus=64,
        num_gpus=int(os.environ['NUM_GPUs']) * 2,
        object_store_memory=128 * 1024**3,
        object_spilling_directory="/root/.cache/ray_tmp/spill",
    )
