...
def processed_dataset(ds):
    ...
    for chat in ds['conversation']:
        ...
        input_chat[-1]['content']= [ input_chat[-1]['content'],(
            {"type": "image",
             "image": "/home/gbrun/LLM_Inference_Power/Nvidia/cat.jpg",
            })]

        # Remove outliers that break models with maximum sequence length of 4096
        if in_len > 4000 or in_len < 64 or out_len > 4000 or out_len < 64:
            continue
...
llm = load_model(model_name,batch_size, tp_size = args.tp, pp_size = args.pp, ep = args.ep)
...
latency_data.to_csv(f"../Results/Single_Runs/{model_name.split('/')[-1]}_{args.tp}gpus_bs{batch_size}.csv", index=True)
